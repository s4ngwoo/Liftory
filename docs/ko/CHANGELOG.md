...
- 시스템 테마 인셋 연동 및 상태바 다크 아이콘 강제: 밝은 톤 Bento 테마(`#FDF7FF`)에 맞춰 `WindowInsetsControllerCompat`를 통해 상태바 및 네비게이션바 아이콘을 항상 어두운 색상으로 설정하여, 기기 OS가 다크 모드일 때도 시스템 아이콘(시계, 배터리 등)의 완벽한 시인성 확보.
